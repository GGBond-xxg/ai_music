...
class LocalMusicService {
  ...
  Future<List<MusicTrack>> pickAudioFolder() async {
    ...
    final folderPath = await FilePicker.platform.getDirectoryPath(
      dialogTitle: 'local.folderTitle'.tr,
    ...
    final fallback = await FilePicker.platform.pickFiles(
      ...
      type: FileType.any,
      withData: false,
      dialogTitle: 'local.folderFallbackTitle'.tr,
    );
    ...
    if (fallbackPaths.isEmpty) {
      throw Exception(
        'local.noAudioInFolder'.trParams({'formats': allowed.join(', ')}),
      );
    }
  ...
  Future<void> _ensureAudioPermission() async {
  ...
  Future<MusicTrack> _trackFromPath(
    ...
    try {
      ...
      artist = _cleanText(_readArtist(metadata)) ?? artist ?? 'local.sourceName'.tr;
    ...
    return MusicTrack(
      ...
      artist: artist ?? 'local.sourceName'.tr,
      album: album,
      coverUri: coverUri,
      lyricText: lyricText,
      duration: duration,
