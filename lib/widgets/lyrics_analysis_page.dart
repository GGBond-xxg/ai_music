...
class _LyricsAnalysisPageState extends State<LyricsAnalysisPage>
  ...
  String _getRandomFunnyText() {
    ...
    // 60% 概率使用动态文本，40% 概率使用静态文�?    if (Random().nextDouble() < 0.6 && trackName.isNotEmpty && artistName.isNotEmpty) {
  ...
  Widget _buildAnalysisContent() {
    ...
    // 过滤掉空内容的分析部�?    final List<Widget> analysisWidgets = [];
    ...
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // 添加一个说明文�?          if (analysisWidgets.length < 4)
            Container(
  ...
  // 检查内容是否有效（非空且非null�?  bool _hasContent(dynamic content) {
