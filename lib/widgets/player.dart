...
class _PlayerState extends State<Player> with TickerProviderStateMixin {
  ...
  Future<void> _prefetchImage(String imageUrl) async {
    if (!_isNetworkImage(imageUrl)) return;
  ...
  Widget? _buildCoverImage(String? url, {Key? key, BoxFit fit = BoxFit.cover}) {
    if (url == null || url.trim().isEmpty) return null;
    final value = url.trim();

    if (_isLocalImage(value)) {
      try {
    ...
    if (_isNetworkImage(value)) {
      return CachedNetworkImage(
        ...
        maxWidthDiskCache: (MediaQuery.of(context).size.width * 1.2).toInt(),
  ...
  void _updateThemeIfNeeded(BuildContext context, Map<String, dynamic>? displayTrack) {
    ...
    if (currentImageUrl == null ||
        ...
        currentImageUrl == _lastImageUrl) {
      return;
    }

    final themeImageProvider = _themeImageProviderFor(currentImageUrl, context);
    if (themeImageProvider == null) return;

    if (_isNetworkImage(currentImageUrl)) {
      _prefetchImage(currentImageUrl);
    }

    if (_isThemeUpdating) return;
    ...
    Future.delayed(const Duration(milliseconds: 300), () {
      if (mounted && currentImageUrl == _lastImageUrl) {
        themeProvider.updateThemeFromImage(
          imageProvider: themeImageProvider,
  ...
  ImageProvider? _themeImageProviderFor(String url, BuildContext context) {
    final value = url.trim();
    if (_isNetworkImage(value)) {
  ...
  Widget _buildMainContent(
    ...
    return AnimatedBuilder(
      ...
      child: GestureDetector(
        ...
        onLongPress: () {
          final item = spotify.currentTrack?['item'];
          final source = UiTexts.of(context).sourceNameFromString(
            item?['sourceType']?.toString(),
            fallback: item?['sourceLabel']?.toString(),
          );
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(UiTexts.of(context).playingFrom(source))),
          );
        },
