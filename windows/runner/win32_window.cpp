...
bool Win32Window::Create(const std::wstring& title,
  ...
  // 标题文字设为空后，再清掉左上角图标，让顶部更接近
  // ParticleMusic 那种“内容即窗口”的 PC 观感。
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    ...
    case WM_GETMINMAXINFO: {
      ...
      min_max_info->ptMinTrackSize.y = Scale(768, scale_factor);
      return 0;
    }
...
void Win32Window::UpdateTheme(HWND const window) {
  // PC 版固定深色标题栏，让原生标题区域尽量贴近应用整体 UI。
  BOOL enable_dark_mode = TRUE;
  DwmSetWindowAttribute(window, DWMWA_USE_IMMERSIVE_DARK_MODE,
                        &enable_dark_mode, sizeof(enable_dark_mode));

  // COLORREF 格式为 0x00BBGGRR。
  DWORD caption_color = 0x001B1015;  // #15101B
  DWORD text_color = 0x00F4ECFF;     // #FFEFF4 近似浅色文字
